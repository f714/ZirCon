...
if ($type == "individual") {
    ...
    $sql = "select id, title, recitation_target, recitation_completed from individual_rooms where owner_id=" . $owner_id;
    ...
    /*for recent book list*/
    $sql = "select id, title, recitation_target, recitation_completed from community_rooms where owner_id=" . $owner_id;
    $rooms = mysqli_query($con, $sql);
    if (mysqli_num_rows($rooms) > 0) {
        /*data login successfully*/
        while ($r = mysqli_fetch_array($rooms)) {
